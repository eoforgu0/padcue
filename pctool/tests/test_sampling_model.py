...
**なぜこのテストがあるか**: 設計文書 A-1 の図では、「2F 押す」が 1 回しか
サンプリングされない例に**「欠落」**というラベルが付いている。これを
「入力そのものが消える」と読むと、2 フレームの押下にまで警告を出す閾値に
なる。実際の A-1 本文は「N の指示に対し実際は **N−1〜N+1**」であり、下限が
0 になるのは N=1 のときだけ。ここではその主張を数え上げで検証し、閾値が
広げられたら落ちるようにしておく。
