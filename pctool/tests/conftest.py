...
pytest の慣習として、検査モジュールは互いに import しない。共有するものは
...
def gcc() -> str:
    ...
    主張で、gcc の無い環境で黙って skip すると「検査は通るのに何も検証していない」
    状態になる(この skip で飛ぶのは 68 件。実測)。CI では
    要求フラグを立てる。
    """
    found = find_gcc()
    if found:
...
def _build_host(gcc: str, name: str, sources: list[str]) -> Path:
    ...
    の検査ごと落とすことがある(ウイルス対策ソフトの定義更新で起こりうる)。
