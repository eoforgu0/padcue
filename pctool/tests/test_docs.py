...
決まりを文章で書いても、守れているかを人が見ている限り抜ける。実際、
コードフェンスの言語付けは「引用(>)の中のフェンスを数えていなかった」
ために2件が抜け、用語集の表は区切り行の1セルが空で表として描画されて
いなかった(どちらも 2026-08-14 に外部レビューで発覚)。同じ抜け方を
二度しないよう、決まりの側を検査にする。
"""
