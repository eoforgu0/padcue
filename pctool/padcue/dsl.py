"""手順 DSL のパーサ・コンパイラ・リント。

意味論(ブロックモデル / 2026-07-29 改定):
...
# 実測(2026-08-01・体感測定): 変化が 13 生値未満の状態が約 70F 続くと作動
# (70F では戻りなし、75F で弱い戻り、90F で明確な戻り)。余裕を持って 60F で
# 警告する(ユーザー決定)。数値は特定ゲームでの測定であり普遍とは限らない
...
def _compile_part(st: Stmt, ctx: _Ctx) -> None:
    ...
    **部品はそこだけで完結する**(2026-08-02 ユーザー指摘)。部品を抜けたら、
    ...
    if ctx.state_tuple() != before:
        ...
        # 素通りする(戻した後は 0 なので、手順末尾の検査にも掛からない)
