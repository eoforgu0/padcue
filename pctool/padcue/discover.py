...
**この順序を入れ替えてはいけない。** 名前解決で得たアドレスは個体IDを伴わないため、
先に入れると同じ装置の「ID付きの応答」を隠してしまい、記録したIDとの照合が
不一致と判定される(詳しくは discover() の説明)。
"""
