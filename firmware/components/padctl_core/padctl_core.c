...
static uint32_t crc32_header_and_recs(const uint8_t *data, size_t rec_len) {
    ...
    const uint8_t *parts[2] = { data, data + PADCTL_HEADER_SIZE };
    size_t lens[2] = { PADCTL_HEADER_SIZE - 4, rec_len };
    for (int k = 0; k < 2; k++) {
...
void padctl_state_neutral(padctl_state_t *st) {
    ...
    st->ax = PADCTL_REST_AX;
    st->ay = PADCTL_REST_AY;
    st->az = PADCTL_REST_AZ;
}

padctl_err_t padctl_decode(const uint8_t *data, size_t len, padctl_proc_t *out) {
    if (len < PADCTL_HEADER_SIZE) return PADCTL_ERR_SHORT;
    if (memcmp(data, "PDT0", 4) != 0) return PADCTL_ERR_MAGIC;
    if (le16(data + 4) != PADCTL_SCHEMA_VERSION) return PADCTL_ERR_SCHEMA;
    uint32_t count = le32(data + 38);
    uint32_t total = le32(data + 42);
    uint32_t crc = le32(data + 46);
    size_t rec_len = len - PADCTL_HEADER_SIZE;
    if (rec_len != (size_t)count * PADCTL_RECORD_SIZE) return PADCTL_ERR_LENGTH;
    if (crc32_header_and_recs(data, rec_len) != crc) return PADCTL_ERR_CRC;

    const uint8_t *recs = data + PADCTL_HEADER_SIZE;
    for (uint32_t i = 0; i < count; i++) {
        const uint8_t *r = recs + (size_t)i * PADCTL_RECORD_SIZE;
        uint8_t op = r[4];
        if (op < OP_STATE || op > OP_AWAIT) return PADCTL_ERR_OPCODE;
        if (op == OP_AWAIT) {
            uint8_t n = r[10];
            if (n < 1 || n > PADCTL_MAX_ARMS) return PADCTL_ERR_OPCODE;
            for (uint8_t k = 0; k < n; k++) {
                uint32_t t = le32(r + 11 + k * 4);
                // 待機分岐は前方へのみ(時間を消費しない閉路を作らない)
                if (t >= count || t <= i) return PADCTL_ERR_ZERO_CYCLE;
            }
            continue;
        }
        if (op == OP_DJNZ || op == OP_JMP) {
            uint32_t target = (op == OP_DJNZ) ? le32(r + 6) : le32(r + 5);
            if (target >= count) return PADCTL_ERR_TARGET;
            if (op == OP_JMP && target <= i) return PADCTL_ERR_ZERO_CYCLE;
            if (op == OP_DJNZ && target <= i && le32(r + 10) == 0) {
                return PADCTL_ERR_ZERO_CYCLE;
            }
        }
    }
    ...
    if (!utf8_valid(data + 6, name_len)) return PADCTL_ERR_NAME_UTF8;
...
padctl_err_t padctl_engine_init(padctl_engine_t *e, const padctl_proc_t *p,
    ...
    return padctl_engine_init_at(e, p, session_loops, 0, 0);
}
...
padctl_err_t padctl_engine_init_at(padctl_engine_t *e, const padctl_proc_t *p,
    ...
    if (start_index >= p->count) return PADCTL_ERR_TARGET;
    ...
    if (start_index != 0) {           // 先頭は手順の自然な開始位置なので検証不要
        ...
        while (i < p->count
               && p->recs[(size_t)i * PADCTL_RECORD_SIZE + 4] == OP_SETCNT) {
            i++;
        }
        if (i >= p->count
            || p->recs[(size_t)i * PADCTL_RECORD_SIZE + 4] != OP_STATE) {
            return PADCTL_ERR_TARGET;
        }
        // 再開点を時刻 0 に寄せる(飛ばした前半ぶん何も出さずに待たないため)
        skip = start_base + le32(p->recs + (size_t)i * PADCTL_RECORD_SIZE);
        if (skip > p->total_frames) return PADCTL_ERR_TARGET;
    }
    memset(e, 0, sizeof(*e));
    // 全ゼロの state は自由落下(az=0)を意味してしまう。ニュートラル
    // (重力あり)へ直す。最初の送出は必ず STATE/AWAIT なので現状この値が
    // 線に乗ることはないが、「0 埋め = ニュートラル」という誤解を残さない
    padctl_state_neutral(&e->state);
    e->recs = p->recs;
    e->count = p->count;
...
padctl_err_t padctl_engine_select(padctl_engine_t *e, uint8_t arm,
    ...
    if (!e->awaiting) return PADCTL_ERR_NO_END;
    if (arm >= e->await_arm_count) return PADCTL_ERR_TARGET;
...
padctl_err_t padctl_engine_step(padctl_engine_t *e, bool *emitted,
                                uint64_t *abs_frame) {
    *emitted = false;
    if (e->done) return PADCTL_OK;
    if (e->awaiting) return PADCTL_OK;   // 選択待ち。時間を刻まない
    if (e->idx >= e->count) return PADCTL_ERR_NO_END;
    const uint8_t *r = e->recs + (size_t)e->idx * PADCTL_RECORD_SIZE;
    uint8_t op = r[4];
    switch (op) {
    case OP_AWAIT: {
        // 全ニュートラルにして止まる。腕は padctl_engine_select で選ぶ
        uint64_t raw = e->base + le32(r) + e->shift;
        if (raw < e->skip) return PADCTL_ERR_TIME_REGRESS;
        uint64_t abs = raw - e->skip;
        if (e->last_frame_valid && abs < e->last_frame) {
            return PADCTL_ERR_TIME_REGRESS;
        }
        e->last_frame = abs;
        ...
        for (uint8_t k = 0; k < e->await_arm_count && k < PADCTL_MAX_ARMS; k++) {
    ...
    case OP_DJNZ: {
        uint8_t c = r[5];
        if (!e->counter_init[c]) return PADCTL_ERR_COUNTER;
        e->counters[c]--;
        e->base += le32(r + 10);
