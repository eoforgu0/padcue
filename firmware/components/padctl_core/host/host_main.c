...
// 使い方: padctl_host <file.bin> <session_loops> <max_steps>
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    padctl_proc_t proc;
    padctl_err_t err = padctl_decode(data, (size_t)len, &proc);
    if (err != PADCTL_OK) { printf("ERR decode %d\n", (int)err); return 3; }

    padctl_engine_t eng;
    uint32_t start_index = (argc == 6) ? (uint32_t)strtoul(argv[4], NULL, 10) : 0;
    uint64_t start_base = (argc == 6) ? strtoull(argv[5], NULL, 10) : 0;
    err = padctl_engine_init_at(&eng, &proc, (uint32_t)strtoul(argv[2], NULL, 10),
                                start_index, start_base);
    if (err != PADCTL_OK) { printf("ERR init %d\n", (int)err); return 3; }

    // 待機分岐の検証用: 環境変数で「選ぶ腕」と「待った時間」を渡す
    const char *choices = getenv("PADCTL_CHOICES");   // 例 "0,1"
    unsigned long await_frames = 0;
    const char *af = getenv("PADCTL_AWAIT_FRAMES");
    ...
    for (unsigned long steps = 0; !eng.done; steps++) {
        ...
        if (eng.awaiting) {
            ...
            err = padctl_engine_select(&eng, arm, await_frames);
            if (err != PADCTL_OK) { printf("ERR select %d\n", (int)err); return 3; }
            continue;
        }
        bool emitted = false;
        uint64_t abs = 0;
        err = padctl_engine_step(&eng, &emitted, &abs);
        if (err != PADCTL_OK) { printf("ERR step %d\n", (int)err); return 3; }
        if (emitted) {
