// padctl_tx: IN エンドポイント送出の仲裁(USB 非依存の純粋C)
//
...
#define PADCTL_TX_REPORT_MAX 64
#define PADCTL_TX_QUEUE_DEPTH 4
// 同じ応答の再送をあきらめる回数。エンドポイントが空いている(tud_hid_ready)
// のに失敗し続けるのは異常なので、キューを詰まらせずに捨てて数える
#define PADCTL_TX_MAX_RETRY 8

// 定期入力レポートの生成関数(方式ごとに異なる: プロコン 0x30 / HID パッド 8B)
typedef size_t (*padctl_tx_build_fn)(void *ctx, const padctl_state_t *st,
                                     uint8_t *out);

typedef struct {
    uint8_t buf[PADCTL_TX_QUEUE_DEPTH][PADCTL_TX_REPORT_MAX];
    uint8_t len[PADCTL_TX_QUEUE_DEPTH];
    uint8_t head;
    uint8_t tail;
    uint8_t count;
...
// padctl_tx_commit() を呼ぶこと。取り出してから送出に失敗すると、その応答は
...
//   捨てられない)。ただし PADCTL_TX_MAX_RETRY 回続けて失敗したら捨てて数える
