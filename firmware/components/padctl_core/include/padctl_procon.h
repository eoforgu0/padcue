// padctl_procon: Pro コントローラー互換プロトコル(USB 非依存の純粋C)
//
...
#define PADCTL_PROCON_REPORT_SIZE 64

// ハンドシェイク・初期化の到達段階(未検証事項3の切り分け用ブレッドクラム)
typedef enum {
    PADCTL_BC_HS_STATUS   = 1u << 0,  // 0x80 0x01 受信
    PADCTL_BC_HS_SHAKE    = 1u << 1,  // 0x80 0x02 受信
    PADCTL_BC_HS_BAUD     = 1u << 2,  // 0x80 0x03 受信
    PADCTL_BC_HS_HIDONLY  = 1u << 3,  // 0x80 0x04 受信
    PADCTL_BC_SUB_DEVINFO = 1u << 4,  // サブコマンド 0x02
    PADCTL_BC_SUB_MODE    = 1u << 5,  // 0x03 入力レポートモード設定
    PADCTL_BC_SUB_SPI     = 1u << 6,  // 0x10 SPI 読み出し
    PADCTL_BC_SUB_LED     = 1u << 7,  // 0x30 プレイヤーLED
    PADCTL_BC_SUB_IMU     = 1u << 8,  // 0x40 IMU 有効化
    PADCTL_BC_SUB_RUMBLE  = 1u << 9,  // 0x48 振動有効化
    PADCTL_BC_INPUT_SENT  = 1u << 10, // 0x30 入力レポートを1回以上送出
} padctl_procon_bc_t;
...
// 応答が必要なら resp(PADCTL_PROCON_REPORT_SIZE バイト)へ書き、その長さを返す。
// 応答不要なら 0 を返す。
//
...
// 応答は padctl_tx_push_reply() へ積み、送出は padctl_tx_next() 経由で行う
