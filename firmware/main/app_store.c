...
// 一覧は RAM に持つ。
// 求められるたびに保存済みの手順を全部フラッシュから読み直してハッシュを
// 計算する作りにはしない。PC は毎秒この一覧を取りに来るので、実行中も毎秒
// フラッシュを読むことになり、実害が2つ出る:
//   (1) フラッシュ操作中はキャッシュが切られるため、その窓に割り込みが重なると
//       パニックする(実行の数%規模。根本原因の IRAM 指定漏れは別途直してある)
//   (2) その間 USB のレポート送出が止まる(毎秒 20〜30ms)= 入力精度の劣化
